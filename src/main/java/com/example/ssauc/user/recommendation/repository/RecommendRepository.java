...
public interface RecommendRepository extends JpaRepository<Product, Long> {
    @Query("SELECT new com.example.ssauc.user.recommendation.dto.RecommendationDto(" +
            "p.productId, p.name, p.description, p.price, u.location, p.imageUrl) " +
            "FROM Product p " +
            "JOIN p.seller u " +
            "WHERE p.endAt >= current_timestamp AND p.status = '판매중'")
    List<RecommendationDto> findRecommendProductsWithoutLogin();  // 입찰마감, 판매 중인 상품만 가져온다.
}