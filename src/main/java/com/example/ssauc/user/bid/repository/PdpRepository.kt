...
interface PdpRepository : JpaRepository<Product?, Long?> {
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT p FROM Product p WHERE p.productId = :productId")
    fun findProductForUpdate(@Param("productId") productId: Long?): Optional<Product?>?


    @Query(
        ("SELECT new com.example.ssauc.user.bid.dto.ProductInformDto("
    ...
    fun getPdpInform(@Param("productId") productId: Long?): ProductInformDto?

    @Modifying
    @Transactional
    @Query("UPDATE Product p SET p.tempPrice = :tempPrice, p.bidCount = p.bidCount + 1 WHERE p.productId = :productId")
    fun updateProductField(@Param("tempPrice") tempPrice: Long?, @Param("productId") productId: Long?): Int


    @Modifying
    @Transactional
    @Query("UPDATE Product p SET p.endAt = :newEndAt WHERE p.productId = :productId")
    fun updateEndAt(@Param("productId") productId: Long?, @Param("newEndAt") newEndAt: LocalDateTime?)
}